...
function runSyncTests() {
  ...
  t('onEditMarkDirty multi-cell clear stays a no-op (the backstop reconciles it)', () => {
    reset([['2026-01-15', '', '', 'PREV-EX', 'PREV-WT', '', '', '', '', '']]);
  ...
  t('onEditMarkDirty writes nothing back into a row cleared of everything', () => {
    reset([
      ['2026-01-15', '185', '135x5', 'PREV-EX', 'PREV-WT', '["ex/E1"]', '', '', '', ''],
      ['2026-01-16', '186', '225x5', 'PREV-EX', 'PREV-WT', '[]', '', '', '', '']
    ]);
    for (let c = 1; c <= HEADERS.length; c++) SHEET.getRange(2, c).setValue('');
    ok(onEditMarkDirty({ range: SHEET.getRange(2, 1, 1, HEADERS.length) }) === false, 'no-op');
    eq(SHEET.getRange(2, 1, 1, HEADERS.length).getValues()[0].join('|'),
      new Array(HEADERS.length).fill('').join('|'), 'the cleared row stayed empty');
    eq(SHEET.getLastRow(), 3, 'still one trailing data row, no phantom row 2');
  });
  ...
  t('onEditMarkDirty a paste spanning blank exercise columns is not an exercise edit', () => {
    reset([['', '', '', 'PREV-EX', 'PREV-WT', '', '', '', '', '']]);
    SHEET.getRange(2, COL['Date']).setValue('2026-01-15');
    SHEET.getRange(2, COL['Weight']).setValue('185');
    ok(onEditMarkDirty({ range: SHEET.getRange(2, COL['Date'], 1, 3) }) === true, 'Date..Bench pasted');
    eq(cell('Weight Synced At'), '', 'weight synced cleared');
    eq(cell('Exercise Synced At'), 'PREV-EX', 'exercise synced untouched');
    eq(cell('Exercises Last Edited At'), '', 'blank exercise cell did not advance the edit timestamp');
  });
  ...
  t('onEditMarkDirty clamps a range reaching past the data and skips one entirely below it', () => {
    reset([['2026-01-15', '', '135x5', 'PREV-EX', 'PREV-WT', '', '', '', '', '']]);
    ok(onEditMarkDirty({ range: SHEET.getRange(2, COL['Bench'], 999, 1) }) === true, 'data row still marked');
    eq(cell('Exercise Synced At'), '', 'the one data row was cleared');
    eq(SHEET.getRange(3, COL['Exercises Last Edited At']).getValue(), '',
      'no marker written into the blank row below the data');
    ok(onEditMarkDirty({ range: SHEET.getRange(50, COL['Bench'], 10, 1) }) === false,
  ...
  t('onEditMarkDirty marks only the rows that had content, not the whole range', () => {
    reset([
      ['2026-01-15', '', '', 'PREV-EX', 'PREV-WT', '', 'FIRST-2', 'LAST-2', '', ''],
      ['2026-01-16', '', '', 'PREV-EX', 'PREV-WT', '', 'FIRST-3', 'LAST-3', '', ''],
      ['2026-01-17', '', '', 'PREV-EX', 'PREV-WT', '', 'FIRST-4', 'LAST-4', '', '']
    ]);
    SHEET.getRange(2, COL['Bench']).setValue('135x5');   // only row 2 gets content
    ok(onEditMarkDirty({ range: SHEET.getRange(2, COL['Bench'], 3, 1) }) === true, 'marked');
    eq(SHEET.getRange(2, COL['Exercise Synced At']).getValue(), '', 'row 2 dirtied');
    ok(SHEET.getRange(2, COL['Exercises Last Edited At']).getValue() !== 'LAST-2', 'row 2 advanced');
    for (const r of [3, 4]) {
      eq(SHEET.getRange(r, COL['Exercise Synced At']).getValue(), 'PREV-EX',
        'row ' + r + ' stayed synced');
      eq(SHEET.getRange(r, COL['Exercises Last Edited At']).getValue(), 'LAST-' + r,
        'row ' + r + ' kept its real last-edit time');
      eq(SHEET.getRange(r, COL['Exercise First Edited At']).getValue(), 'FIRST-' + r,
  ...
  t('onEditMarkDirty leaves an unmarked row sitting between two marked ones alone', () => {
    reset([
      ['2026-01-15', '', '', 'PREV-EX', 'PREV-WT', '', 'FIRST-2', 'LAST-2', '', ''],
      ['2026-01-16', '', '', 'PREV-EX', 'PREV-WT', '', 'FIRST-3', 'LAST-3', '', ''],
      ['2026-01-17', '', '', 'PREV-EX', 'PREV-WT', '', 'FIRST-4', 'LAST-4', '', '']
    ...
    ok(onEditMarkDirty({ range: SHEET.getRange(2, COL['Bench'], 3, 1) }) === true, 'marked');
    eq(SHEET.getRange(2, COL['Exercise Synced At']).getValue(), '', 'row 2 dirtied');
    eq(SHEET.getRange(4, COL['Exercise Synced At']).getValue(), '', 'row 4 dirtied');
    eq(SHEET.getRange(3, COL['Exercise Synced At']).getValue(), 'PREV-EX', 'the gap row stayed synced');
    eq(SHEET.getRange(3, COL['Exercises Last Edited At']).getValue(), 'LAST-3', 'gap row kept its last-edit time');
    eq(SHEET.getRange(3, COL['Exercise First Edited At']).getValue(), 'FIRST-3', 'gap row kept its anchor');
  });
  ...
  t('onEditMarkDirty marks only the rows whose bodyweight changed', () => {
    ...
    ok(onEditMarkDirty({ range: SHEET.getRange(2, COL['Weight'], 2, 1) }) === true, 'marked');
    eq(SHEET.getRange(3, COL['Weight Synced At']).getValue(), '', 'row 3 dirtied');
    ok(SHEET.getRange(3, COL['Weight Edited At']).getValue() !== 'WEDIT-3', 'row 3 advanced');
    eq(SHEET.getRange(2, COL['Weight Synced At']).getValue(), 'PREV-WT', 'row 2 stayed synced');
    eq(SHEET.getRange(2, COL['Weight Edited At']).getValue(), 'WEDIT-2', 'row 2 kept its edit time');
  });
  ...
  t('onEditMarkDirty keeps the two phases on their own rows within one range', () => {
    reset([
      ['2026-01-15', '', '', 'PREV-EX', 'PREV-WT', '', '', 'LAST-2', 'WEDIT-2', ''],
      ['2026-01-16', '', '', 'PREV-EX', 'PREV-WT', '', '', 'LAST-3', 'WEDIT-3', '']
    ]);
    SHEET.getRange(2, COL['Weight']).setValue('186');    // row 2: weight only
    SHEET.getRange(3, COL['Bench']).setValue('135x5');   // row 3: exercise only
    ok(onEditMarkDirty({ range: SHEET.getRange(2, COL['Weight'], 2, 2) }) === true, 'marked');
    eq(SHEET.getRange(2, COL['Weight Synced At']).getValue(), '', 'row 2 weight dirtied');
    eq(SHEET.getRange(2, COL['Exercise Synced At']).getValue(), 'PREV-EX', 'row 2 exercise untouched');
    eq(SHEET.getRange(2, COL['Exercises Last Edited At']).getValue(), 'LAST-2', 'row 2 exercise time kept');
    eq(SHEET.getRange(3, COL['Exercise Synced At']).getValue(), '', 'row 3 exercise dirtied');
    eq(SHEET.getRange(3, COL['Weight Synced At']).getValue(), 'PREV-WT', 'row 3 weight untouched');
    eq(SHEET.getRange(3, COL['Weight Edited At']).getValue(), 'WEDIT-3', 'row 3 weight time kept');
  });
  ...
  t('readRows refuses duplicate exercise column headers', () => {
    ...
    let thrown = null;
    try { withStubs(NO_FOREIGN, () => syncDirtyRows(0)); } catch (err) { thrown = err; }
    ok(thrown !== null, 'throws');
    ok(String(thrown).indexOf('Bench') !== -1, 'names the duplicated header: ' + thrown);
    ok(PROPS.getProperty('pendingDirty') !== null, 'dirty flag kept for after the fix');
  });
  ...
  t('syncDirtyRows attempts every ready row and leads the summary with the counts', () => {
    reset(outageGrid(8));
    let created = 0;
    let thrown = null;
    withStubs(Object.assign({}, NO_FOREIGN, {
  ...
  t('syncDirtyRows processes at most MAX_ROWS_PER_SYNC rows and defers the rest', () => {
    const extra = 2;
    const day = 24 * 60 * 60 * 1000;
    const now = Date.now();
    const grid = [];
    ...
    ok(PROPS.getProperty('pendingDirty') !== null, 'flag kept for the deferred backlog');
  });
  ...
  t('syncDirtyRows recreates rather than skipping when a row has two prior exercise ids', () => {
    ...
    eq(cell('Created Health IDs'), JSON.stringify(['users/me/dataTypes/exercise/dataPoints/E3']));
  });
  ...
  t('syncDirtyRows deletes without recreating when the exercise content is gone', () => {
  ...
  t('syncDirtyRows PATCHes a re-synced bodyweight in place and keeps the resource name', () => {
    ...
    eq(cell('Created Health IDs'), JSON.stringify([wName]), 'resource name unchanged');
  ...
  t('syncDirtyRows skips the PATCH and stays dirty when the prior-weight GET fails', () => {
    const wName = 'users/me/dataTypes/weight/dataPoints/W1';
    reset([['2026-01-15', '186', '', 'SYNC', '', JSON.stringify([wName]), '', '', '', '']]);
    const calls = [];
    const r = withStubs(Object.assign({}, NO_FOREIGN, {
      getDataPoint: () => { throw new Error('simulated 500'); },
      patchWeight: () => { calls.push('patch'); }
    }), () => syncDirtyRows(0));
    eq(r.errors, 1, 'row counted as failed');
    eq(calls, [], 'no PATCH attempted');
    eq(cell('Weight Synced At'), '', 'row left dirty for the next pass');
    ok(PROPS.getProperty('pendingDirty') !== null, 'flag kept so the next poll retries');
  });
  ...
  t('syncDirtyRows drops a 404 prior weight id and POSTs a fresh datapoint', () => {
    ...
    eq(cell('Created Health IDs'),
      JSON.stringify(['users/me/dataTypes/weight/dataPoints/W-new']), 'stale id replaced');
    ok(cell('Weight Synced At') !== '', 'weight stamped');
  });
  ...
  t('syncDirtyRows clears the recorded foreign session when a row no longer matches', () => {
    reset([[JAN15_NOON_EST, '', '135x5x3', '', 'SYNC', '', JAN15_5PM_EST, JAN15_530PM_EST, '', 'foreign/OLD']]);
    withStubs(Object.assign({}, NO_FOREIGN, {
  ...
  t('syncOneRow_ defers the Exercise Synced At stamp when an exercise edit lands mid-pass', () => {
    const editedAt = new Date(Date.UTC(2026, 0, 15, 22, 0, 0));
    reset([['2026-01-15', '', '135x5x3', '', 'SYNC', '', '', editedAt, '', '']]);
    const r = withStubs(Object.assign({}, NO_FOREIGN, {
      createExerciseAt: () => {
        SHEET.getRange(2, COL['Exercises Last Edited At'])
          .setValue(new Date(editedAt.getTime() + 60 * 1000));
        return 'users/me/dataTypes/exercise/dataPoints/E1';
      }
    }), () => syncDirtyRows(0));
    eq(r.ok, 1, 'the push itself succeeded');
    eq(cell('Exercise Synced At'), '', 'stamp deferred, so the row stays dirty');
    eq(cell('Created Health IDs'),
      JSON.stringify(['users/me/dataTypes/exercise/dataPoints/E1']), 'id still persisted');
    ok(PROPS.getProperty('pendingDirty') !== null, 'flag kept so the next poll re-pushes');
  });
  ...
  t('syncOneRow_ defers the Weight Synced At stamp when a weight edit lands mid-pass', () => {
    const editedAt = new Date(Date.UTC(2026, 0, 15, 22, 0, 0));
    reset([['2026-01-15', '185', '', 'SYNC', '', '', '', '', editedAt, '']]);
    const r = withStubs(Object.assign({}, NO_FOREIGN, {
      createWeightAt: () => {
        SHEET.getRange(2, COL['Weight Edited At'])
          .setValue(new Date(editedAt.getTime() + 60 * 1000));
        return 'users/me/dataTypes/weight/dataPoints/W1';
      }
    }), () => syncDirtyRows(0));
    eq(r.ok, 1, 'the push itself succeeded');
    eq(cell('Weight Synced At'), '', 'stamp deferred, so the row stays dirty');
    eq(cell('Created Health IDs'),
      JSON.stringify(['users/me/dataTypes/weight/dataPoints/W1']), 'id still persisted');
    ok(PROPS.getProperty('pendingDirty') !== null, 'flag kept so the next poll re-pushes');
  });
  ...
  t('a multi-cell clear that onEdit ignores is reconciled by the backstop', () => {
    const exName = 'users/me/dataTypes/exercise/dataPoints/E1';
    const wtName = 'users/me/dataTypes/weight/dataPoints/W1';
    reset([[new Date(), '185', '135x5x3', 'SYNC', 'SYNC', JSON.stringify([exName, wtName]), '', '', '', '']]);
    ...
    ok(onEditMarkDirty({ range: SHEET.getRange(2, COL['Weight'], 1, 2) }) === false,
      'onEdit ignores the multi-cell clear');
    eq(cell('Exercise Synced At'), 'SYNC', 'still stamped after the edit');

    withStubs({ listStrengthOnDate: () => [], listWeightOnDate: () => [] }, () => backstop());
    eq(cell('Exercise Synced At'), '', 'backstop re-dirtied the exercise phase');
    eq(cell('Weight Synced At'), '', 'backstop re-dirtied the weight phase');
    ok(PROPS.getProperty('pendingDirty') !== null, 'pending flag set for the poll');
  ...
  t('a paste that blanks the bodyweight while writing another cell is reconciled', () => {
    const wtName = 'users/me/dataTypes/weight/dataPoints/W1';
    reset([[new Date(), '185', '', 'SYNC', 'SYNC', JSON.stringify([wtName]), '', '', '', '']]);
    ...
    eq(cell('Weight Synced At'), 'SYNC', 'onEdit leaves the weight phase stamped');
  ...
  t('a row cleared including its Date keeps its datapoints', () => {
    const exName = 'users/me/dataTypes/exercise/dataPoints/E1';
    reset([
      [new Date(), '185', '135x5x3', 'SYNC', 'SYNC', JSON.stringify([exName]), '', '', '', ''],
      [new Date(Date.now() + 86400000), '', '225x5', 'SYNC', 'SYNC', '[]', '', '', '', '']
    ]);
    for (let c = 1; c <= HEADERS.length; c++) SHEET.getRange(2, c).setValue('');
    const deleted = [];
    withStubs({
  ...
  t('blanking an exercise column header does not make every row look cleared', () => {
    const grid = [];
    for (let i = 5; i >= 1; i--) {
      grid.push([new Date(Date.now() - i * 24 * 60 * 60 * 1000), '185', '135x5x3', 'SYNC', 'SYNC',
    ...
    withStubs({ listStrengthOnDate: () => [], listWeightOnDate: () => [] }, () => backstop());
    for (let r = 2; r <= 6; r++) {
      eq(SHEET.getRange(r, COL['Exercise Synced At']).getValue(), 'SYNC',
        'row ' + r + ' still stamped; its cells still hold text');
    }
    ok(PROPS.getProperty('pendingDirty') === null, 'nothing queued for deletion');
  });

  // Same shape via the weight parser: a reformat it rejects returns null, which
  // is not the same claim as an empty cell.
  t('a bodyweight the parser rejects does not look like a cleared bodyweight', () => {
    reset([[new Date(), '185 lb', '', 'SYNC', 'SYNC',
      JSON.stringify(['users/me/dataTypes/weight/dataPoints/W1']), '', '', '', '']]);
    eq(parseBodyweight('185 lb'), null, 'precondition: the parser rejects it');
    withStubs({ listStrengthOnDate: () => [], listWeightOnDate: () => [] }, () => backstop());
    eq(cell('Weight Synced At'), 'SYNC', 'datapoint left alone; the cell still holds text');
  });
  ...
  t('backstop reconciles nothing when more rows look cleared than a person would clear', () => {
    const grid = [];
    for (let i = STALE_RECONCILE_MAX_ROWS + 1; i >= 1; i--) {
      grid.push([new Date(Date.now() - i * 24 * 60 * 60 * 1000), '', '', 'SYNC', 'SYNC',
        JSON.stringify(['users/me/dataTypes/exercise/dataPoints/E' + i]), '', '', '', '']);
    }
    reset(grid);
    withStubs({ listStrengthOnDate: () => [], listWeightOnDate: () => [] }, () => backstop());
    for (let r = 2; r <= grid.length + 1; r++) {
      eq(SHEET.getRange(r, COL['Exercise Synced At']).getValue(), 'SYNC', 'row ' + r + ' untouched');
    }
  ...
  t('backstop still reconciles a stale set at the limit', () => {
    ...
    reset(grid);
    withStubs({ listStrengthOnDate: () => [], listWeightOnDate: () => [] }, () => backstop());
    eq(SHEET.getRange(2, COL['Exercise Synced At']).getValue(), '', 'reconciled at the limit');
  });
  ...
  t('backstop reconciles a cleared row older than the foreign-match lookback', () => {
    const exName = 'users/me/dataTypes/exercise/dataPoints/E-old';
    const old = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
    reset([[old, '', '', 'SYNC', 'SYNC', JSON.stringify([exName]), '', '', '', '']]);
    withStubs({ listStrengthOnDate: () => [], listWeightOnDate: () => [] }, () => backstop());
    eq(cell('Exercise Synced At'), '', 'stale row re-dirtied regardless of age');
  });
  ...
  t('backstop leaves a consistent row alone', () => {
    const exName = 'users/me/dataTypes/exercise/dataPoints/E1';
    const old = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
    reset([[old, '185', '135x5x3', 'SYNC', 'SYNC', JSON.stringify([exName]), '', '', '', '']]);
    withStubs({ listStrengthOnDate: () => [], listWeightOnDate: () => [] }, () => backstop());
