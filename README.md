...
- **Idempotent sync**: Each pass reconciles a row's Health datapoint(s) with current cell content. Editing a synced row clears its `Synced At` stamps and reprocesses it. The exercise datapoint is delete+recreated **only when the interval or notes changed**; an unchanged re-sync keeps the existing datapoint and costs just a read. (The Health API has no verified update path for exercise datapoints, so delete+POST is the only update mechanism. `PATCH` was a server-side no-op for every field; the API team reports `activeDuration` fixed as of 2026-08, with `notes` and `interval` still unmeasured, see [Google Cloud caveats](#google-cloud-caveats).)
...
- **`exercise.displayName` is server-owned.** For every `exerciseType` except `OTHER`, Health ignores whatever the client sends and derives the title from the type (`STRENGTH_TRAINING` renders as "Strength Training"); only `OTHER` keeps a custom, free-form name. So `exerciseType` is the only lever on the card's title, and the sync sends no `displayName` at all (confirmed by the Health API team, 2026-08).
- **`exercise` PATCH is fixed for `activeDuration` only, so far.** PATCH against an exercise datapoint used to 500 on a partial body and return `200 done:true` while changing nothing on a full one. The API team fixed `activeDuration` in 2026-08 and closed `displayName` as working-as-intended (above); `notes` and `interval` were not mentioned and remain unverified. `debugRunAll()` in [Debug.gs](src/Debug.gs) measures all of them against the live API. Until it says otherwise, the sync keeps delete+recreating exercise datapoints.
