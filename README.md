...
[![Test](https://github.com/andornaut/google-health-sheet-sync/actions/workflows/test.yml/badge.svg)](https://github.com/andornaut/google-health-sheet-sync/actions/workflows/test.yml)
...
- **Idempotent sync**: Each pass reconciles a row's Health datapoint(s) with current cell content. Editing a synced row clears its `Synced At` stamps and reprocesses it. The exercise datapoint is delete+recreated **only when the interval or notes changed**; an unchanged re-sync keeps the existing datapoint and costs just a read. (`PATCH` merges an exercise datapoint's `interval` and nothing else, `notes` included, so delete+POST is the only way to rewrite the notes a change to a row produces. See [Google Cloud caveats](#google-cloud-caveats).)
...
- **Foreign-activity matching**: If a Strength Training session from a watch or other app overlaps the row's edit window (timestamps on the row's `Date`, within 10 min slack), the script borrows that session's start/end times for the datapoint it writes and records the match in `Matched Health Session`. A row claims **every** session its window overlaps, not just the closest one: see multiple workouts on one day below. The script **always writes its own datapoint** (with reps/sets notes); the foreign datapoint is left untouched (the API forbids modifying it anyway). A backstop runs every 4 hours (`BACKSTOP_INTERVAL_HOURS`) and re-reviews exercise rows from the last 2 days (`BACKSTOP_LOOKBACK_DAYS`), both unmatched (to align a late foreign session) and matched (to re-borrow a foreign interval extended after your last edit). This runs off the 5-minute poll, so foreign re-review doesn't query the Health API every 5 minutes; the tradeoff is that a late foreign session aligns within ~4 hours rather than minutes.
- **Multiple workouts on one day**: A day is one row (dates must be in increasing order, so two rows can't share one). When you start and stop two separate workouts in the Google Health app and log an exercise into the sheet during each, the script writes **one datapoint per workout**, each carrying only the exercises typed during it and each borrowing that workout's start/end times. Attribution uses the per-exercise timestamps in `Exercise Edit Times`: an exercise goes to the app session containing the moment you **first** typed it, so a typo corrected during the next workout stays with the workout it belongs to. Exercises that fall in no session go to a single extra datapoint. When every exercise in that group carries a per-exercise timestamp, the group is timed from its own first/last edits (through the same timing rules as a whole row: the on-date requirement, the duration clamps, the fall-back to a prior datapoint's interval), so something typed between two workouts starts when it was typed. An exercise without a per-exercise timestamp falls back to the row-level timestamps for both attribution and timing: a row written before `Exercise Edit Times` existed therefore lands entirely on the first workout, as it did before the split, and a mixed group widens to the row-level window. In the rare case where two groups resolve to the identical interval, only the first is written, because the API treats a second create on the same interval as a no-op that silently discards its notes. Because a workout's session exists in Health only once you stop it, the split for the last workout of the day settles on the next backstop pass rather than immediately.
...
- **`Date`**
- **Exercise columns** (headers are exercise names)
- **`Weight`** (bodyweight in lb)

Column order does not matter. `setup` appends the managed columns after the last existing column and hides them.
...
| Jun 3, 2026  | 175x6x6     | 295x5x5  |        | 2026-06-03T19:00:00Z |                      | [ex/011, ex/012]   | [ex/778, ex/779]       |

The May 24 row was also logged on a watch: the script still wrote its own datapoint (`ex/010`) but borrowed the watch session's start/end times and recorded the watch session (`ex/777`) in `Matched Health Session`. The watch datapoint is untouched.

Jun 3 held two separate workouts in the app. Bench press was typed during the first and Deadlift during the second, so the row carries two datapoints (`ex/011`, `ex/012`), each aligned to its own workout (`ex/778`, `ex/779`) and carrying only that workout's exercise.
...
- **`exercise.displayName` is server-owned.** For every `exerciseType` except `OTHER`, Health ignores whatever the client sends and derives the title from the type; only `OTHER` keeps a custom, free-form name. `STRENGTH_TRAINING` reads back as "Strength training" whatever the client sends. `exerciseType` is therefore the only lever on the card's title, and the sync sends no `displayName` at all (confirmed by the Health API team, 2026-08).
- **`exercise` PATCH now merges `interval`, and nothing else.** Measured against the live API on 2026-08-19: a full-body PATCH applies `interval`, while `notes` and `displayName` read back unchanged, and a partial body still returns `500 INTERNAL`. `notes` no-ops even when the body carries only client-owned fields, so echoing back `createTime` / `updateTime` / `metricsSummary` is not what the server objects to. Because `notes` is the field a row edit changes, exercise datapoints are still delete+recreated.
- **`exercise.activeDuration` is computed, not stored.** The server sets it to the interval's length and ignores the client, on create as well as on PATCH: a create sending `137s` for a 600-second interval reads back `600s`, and a PATCH that lengthens the interval to 1500 seconds moves `activeDuration` to `1500s` on its own. This is worth knowing precisely, because the API team's 2026-08 reply says the field is now editable by PATCH, and it is not: nothing a client sends reaches it. A create that omits the field reads back the interval's length just the same, so this client sends none. The upside of the derivation is that an interval PATCH cannot leave a stale duration behind.
...
- `FOREIGN_MATCH_BUFFER_MS` (10 min): slack on either side when checking whether a foreign Strength Training session overlaps a row's edit window, and the same slack when deciding which session an individual exercise was logged during. An overlapping session only supplies its interval (the script always writes its own datapoint), and every claimed session's name is recorded in `Matched Health Session`.
