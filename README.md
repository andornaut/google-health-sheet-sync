...
- **Automated**: Polls every 5 minutes for dirty rows past the debounce. A daily backstop re-dirties recent unmatched exercise rows so late-arriving foreign sessions get picked up, and deletes any sync-created datapoints that an interrupted run orphaned (no row references them).
