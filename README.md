...
- **`exercise` PATCH is fixed for `activeDuration` only, so far.** PATCH against an exercise datapoint used to 500 on a partial body and return `200 done:true` while changing nothing on a full one. The API team fixed `activeDuration` in 2026-08 and closed `displayName` as working-as-intended (above); `notes` and `interval` were not mentioned and remain unverified. `probeExercisePatch()` in [Probe.gs](src/Probe.gs) measures all of them against the live API. Until it says otherwise, the sync keeps delete+recreating exercise datapoints.
...
- **Probe the live API**: run `probeExercisePatch` from the Apps Script editor's function picker. It creates a throwaway `STRENGTH_TRAINING` datapoint at 03:00 today, PATCHes one field at a time, GETs after each to see whether the change stuck, and deletes it again; the verdicts land in the Executions log. Findings belong in a comment next to the code that relies on them.
