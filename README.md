...
- **`exercise` PATCH now merges `interval`, and nothing else.** Measured against the live API on 2026-08-19 with `debugRunAll()` in [Debug.gs](src/Debug.gs): a full-body PATCH applies `interval` (new: it used to no-op like everything else), while `notes`, `displayName` and `activeDuration` are all read back unchanged, and a partial body still returns `500 INTERNAL` after four retries. Note that this contradicts the API team's 2026-08 reply, which reported `activeDuration` fixed. Because `notes` is the field the sync actually needs to update, exercise datapoints are still delete+recreated.
