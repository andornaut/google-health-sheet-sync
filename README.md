...
- **A corrected value on an old row lost its original time**: What to expect differs by column. **Weight**: typing over the cell PATCHes the existing datapoint and keeps its original sample time, so prefer that; clearing it first does not, because the clear deletes the datapoint and the retype creates a new one at the synthetic noon slot on the row's `Date`. **Exercise**: editing a row that was logged live on its own day rebuilds the session as *first edit → first edit + 2 h* (the cap), however long ago that was. `Exercise First Edited At` is sticky and still matches the row's `Date`, so the mechanism that grows `endTime` while you log sets applies months later too. Typing over instead of clearing does **not** avoid this; nothing does short of not editing the row. Only genuinely backfilled rows (never edited on their own date) keep their recorded interval, and for those, typing over preserves it while clearing first loses it to the synthetic noon slot.
