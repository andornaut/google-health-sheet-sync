...
- **Check the live API**: run `debugRunAll` from the Apps Script editor's function picker ([Debug.gs](src/Debug.gs)). It creates a throwaway `STRENGTH_TRAINING` datapoint at 03:00, PATCHes one field at a time, GETs after each to see whether the change stuck, deletes it again, and logs one redacted report block to copy out. `debugCleanup` collects a datapoint an interrupted run left behind. Findings belong in a comment next to the code that relies on them.
